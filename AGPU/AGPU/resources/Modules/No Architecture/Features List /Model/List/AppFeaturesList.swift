...
struct AppFeaturesList {
    ...
    static let features = [
        ...
        AppFeatureModel(
            id: 2,
            name: "Voice Actions",
            description: "Voice Actions (голосовые действия) - просто используйте свой голос, чтобы открыть нужные разделы на сайте, найти конкретный корпус на карте или даже прокручивать веб-страницы. Вы можете просто сказать \"Вниз\" или \"Вверх\", и страница автоматически будет прокручиваться в соответствующем направлении. Также данная опция позволяет просматривать следующее и предыдущее изображение в AR с помощью голосовых команд.",
        ...
        AppFeatureModel(
            id: 6,
            name: "Adaptive News",
            description: "Adaptive News (адаптивные новости) - предоставляет релевантные новости в зависимости от выбранного факультета. В настройках доступны опции: категория, порядок опций, вид, индикатор загрузки и возможность выделять новости за сегодня. Каждая новость сохраняет позицию для продолжения чтения. Также можно взаимодействовать со списком недавно прочитанных новостей, проверять количество предложений, слов и наличие ключевых слов у выбранной новости. Дополнительно доступны распознавание рукописных цифр для получения новостей и просмотр изображений в дополненной реальности.",
        ...
        AppFeatureModel(
            id: 8,
            name: "Your Status",
            description: "Your Status (ваш статус) - при выборе статуса меняется экран на соответствующей вкладке. На экранах \"Абитуриенту\", \"Студенту\", \"Сотруднику\" вы можете изменить порядок разделов или вернуть его в исходное состояние.",
        ...
        AppFeatureModel(
            ...
            description: "Innovative Timetable (инновационное расписание) - опция, позволяющая получить расписание на основе выбранной группы, аудитории или преподавателя. Она позволяет выбрать неделю из списка, автоматически прокрутиться до нужной недели, дня недели и группы, а также предоставляет информацию о том, в каком корпусе будут проходить занятия. Дополнительно имеется распознавание текста для получения расписание по сфотографированной дате. Кроме того, вы можете отфильтровать пары по типу, чтобы быстро и удобно получить доступ к интересующим вас занятиям. Также у вас есть возможность поделиться расписанием на день или неделю в виде изображения или сохранить изображение расписания в фото вашего устройства. В дополнение к этому, данная функция предоставляет информацию о времени, оставшемся до начала или конца пары. Вместе с тем вы можете взаимодействовать со списком \"Избранное\". Помимо этого у вас есть возможность просматривать расписание в дополненной реальности.",
        ...
        AppFeatureModel(
            id: 10,
            name: "Smart Calendar",
            description: "Smart Calendar (умный календарь) - при выборе даты в календаре отображается информация о расписании или погоде. К тому же у вас есть возможность взаимодействовать со списком недавних дат. Также вы можете посмотреть расписание для нескольких одновременно выбранных дат в календаре.",
        ...
        AppFeatureModel(
            id: 12,
            name: "Only Main",
            description: "Only Main (только главное) - сокращает количество вкладок с 4 до 2, оставляя один из вариантов вкладок: 1) \"Расписание\" и \"Настройки\" 2) \"Новости\" и \"Настройки\" 3) \"Разделы\" и \"Настройки\".",
        ...
        AppFeatureModel(
            id: 15,
            name: "Custom TabBar",
            description: "Custom TabBar (своя панель вкладок) - у вас есть возможность настроить панель вкладок по своему усмотрению. Например, вы можете настроить порядок вкладок, выбрать цвет или включить/выключить анимацию для них, а также выбрать стиль иконок.",
        ...
        AppFeatureModel(
            id: 16,
            name: "Visual Changes",
            description: "Visual Changes (наглядные изменения) - показывает, что изменилось для интересующего вас контента. Например, вы будете знать, какие новости появились за сегодня или как изменилась погода.",
            videoURL: ""
        ),
        ...
        AppFeatureModel(
            id: 17,
            name: "AR Mode",
            description: "AR Mode (AR режим) - позволяет просматривать изображения в дополненной реальности. Например, вы можете увидеть изображение расписания или сохранённое изображение в виде куба. Также у вас есть возможность выбрать форму или плоскость для объекта в AR. Помимо этого вы можете поделиться скриншотом.",
            videoURL: ""
        ),
