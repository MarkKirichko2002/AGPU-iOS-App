...
struct AppFeaturesList {
    ...
    static let features = [
        ...
        AppFeatureModel(
            id: 3,
            name: "Find ASPU",
            description: "Find ASPU (найти АГПУ) - простой и удобный способ быстрого поиска всех корпусов на карте. С помощью этой опции вы сможете легко найти все необходимые корпуса, отображенные на карте, в удобном интерфейсе. Особенно полезно то, что эта опция предоставляет актуальную информацию о погоде и отображает наличие и количество пар на текущий день для выбранного корпуса. Кроме того, вы можете с легкостью поделиться локацией конкретного корпуса или кафедры."
        ),
