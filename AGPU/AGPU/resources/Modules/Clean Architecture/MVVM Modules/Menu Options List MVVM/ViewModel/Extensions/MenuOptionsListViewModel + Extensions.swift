//
//  MenuOptionsListViewModel + Extensions.swift
//  AGPU
//
...
extension MenuOptionsListViewModel: IMenuOptionsListViewModel {
    ...
    func getOptions() {
        options = settingsManager.loadMenuOptions(category: category.rawValue)
        dataChangedHandler?()
    }
    ...
    func updateOptions(options: [MenuOptionModel], _ index: Int, _ index2: Int) {
        ...
        settingsManager.saveMenuOptions(category: category.rawValue, options: arr) {
            self.getOptions()
        }
    }
